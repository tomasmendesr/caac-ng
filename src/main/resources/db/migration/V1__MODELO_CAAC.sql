...
IF OBJECT_ID('caac.DIA') IS NOT NULL
    PRINT '<<< CREATED TABLE caac.DIA >>>'
ELSE
    PRINT '<<< FAILED CREATING TABLE caac.DIA >>>'
go
...
IF OBJECT_ID('caac.ESPACIOS') IS NOT NULL
    PRINT '<<< CREATED TABLE caac.ESPACIOS >>>'
ELSE
    PRINT '<<< FAILED CREATING TABLE caac.ESPACIOS >>>'
go
...
CREATE TABLE caac.HOJA_REUNIONES_EQUIPO(
    idHoja           bigint         NOT NULL,
    si               char(1)        NULL,
    no               char(1)        NULL,
...
IF OBJECT_ID('caac.HOJA_REUNIONES_EQUIPO') IS NOT NULL
    PRINT '<<< CREATED TABLE caac.HOJA_REUNIONES_EQUIPO >>>'
ELSE
...
CREATE TABLE caac.TIPO_INSTITUCION(
...
IF OBJECT_ID('caac.TIPO_INSTITUCION') IS NOT NULL
    PRINT '<<< CREATED TABLE caac.TIPO_INSTITUCION >>>'
ELSE
    PRINT '<<< FAILED CREATING TABLE caac.TIPO_INSTITUCION >>>'
go
...
ALTER TABLE caac.HOJA_ARTICULACION_INST ADD CONSTRAINT FK_TINST_HARTIC 
    ...
    REFERENCES caac.TIPO_INSTITUCION(id_tipo_inst)
go
...
ALTER TABLE caac.HOJA_ESTRUCTURA ADD CONSTRAINT FK_ESPACIOS_HESTRUCTURA 
    ...
    REFERENCES caac.ESPACIOS(idespacio)
go
...
ALTER TABLE caac.HOJA_REUNIONES_EQUIPO ADD CONSTRAINT FK_CLFREC_HREQUIP
    FOREIGN KEY (idfrecuencia)
    REFERENCES caac.CLASIFICACION_FRECUENCIA(id)
go

ALTER TABLE caac.HOJA_REUNIONES_EQUIPO ADD CONSTRAINT [FK_HCAAC_HREUNION]
    FOREIGN KEY (idHoja)
    REFERENCES caac.HOJA_CAAC(idHoja)
go
